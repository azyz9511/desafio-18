...
const productosSchema = require('./modelsMDB/productosSchema')
...
class Product{
    ...
    async connectDB(){
        try{
            const URL = 'mongodb+srv://azyz:asd.456@cluster0.klade.mongodb.net/test?retryWrites=true&w=majority';
    ...
    async editProd(producto){   
        try{
            ...
            if(product.length !== 0){
                await productosSchema.updateOne({id: producto.id},{$set: {
