...
const config = require('./db/ecommerce-5f30e-firebase-adminsdk-nso6z-8f51fd1043.json');
const Product = require('./productosDaos');
// de esta foma no me lo reconoce como clase, no se por que si esta correcto
// por eso lo importe directo y no por indexDaos
// const {productosDaos: Product} = require('./indexDaos'); 
    ...
    async addProdCar(idCar,idProduct){
        ...
        try{
            ...
            if(carritoId !== undefined && productoId.length !== 0){
                ...
                await doc.update('productos', FieldValue.arrayUnion(data),{merge: true});
    ...
    async deleteProdCar(idCar,idProduct){
        ...
        try{
            ...
            if(carritoId !== undefined){
                const productoId = carritoId.productos.find((producto) => producto.id == idProduct);
